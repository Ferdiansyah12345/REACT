...
export default function CreateMahasiswa() {
  ...
  // Mengambil daftar prodi dari API saat komponen dimuat
  useEffect(() => {
    const fetchProdi = async () => {
      try {
        ...
        setProdiList(response.data.data); // Simpan data prodi ke dalam state
      } catch (error) {
        setError("Failed to fetch prodi data");
      }
    };

    fetchProdi(); // Panggil fungsi untuk mengambil data prodi
  ...
  const handleSubmit = async (e) => {
    ...
    // Validasi input: jika namamahasiswa atau prodiId kosong, set pesan error
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          ...
          <select
            ...
            id="prodiId"
            value={prodiId} // Nilai dropdown disimpan di state prodiId
            onChange={(e) => setProdiId(e.target.value)} // Update state saat pilihan berubah
          >
            <option value="">Select Program Studi</option>
            {prodiList.map((prodi) => (
              <option key={prodi.id} value={prodi.id}>
                {/* Set key dan value untuk masing-masing prodi */}
                {prodi.nama} {/* Nama prodi sebagai teks di dropdown */}
              </option>
            ))}
          </select>
        </div>
